# Paper Trade Summary

This file tracks simulated paper positions only. It is not a record of real bets or trades.

## Current focus

- 2026-06-08 00:12 UTC run: no new paper-only positions opened. Current best gaps are maintained edges already represented in the paper book, and localized Polymarket price disagreement is too high for added simulated exposure.
- Houston KHOU Jun 8 88-89F YES remains the top maintained edge: current readable range roughly 32-39c versus fair 42-52%. Maintain PT-20260607-059 from 32c only; do not duplicate unless a verified quote falls materially below entry or KHOU-specific evidence strengthens.
- Miami KMIA Jun 8 90-91F YES remains a represented raw gap: current readable range roughly 20-31c versus fair 34-44%. Maintain PT-20260607-058/PT-20260607-062/PT-20260607-066 only; no fourth add unless a clean quote falls materially below 15c or KMIA-specific evidence strengthens.
- Hong Kong Observatory Jun 8 30C YES remains a represented adjacent-bucket hedge: current readable range roughly 25-34c versus fair 34-44%. Maintain PT-20260607-069 from 26c and PT-20260607-057 on 29C as cluster exposure; do not duplicate while price pages conflict and one-decimal boundary risk remains high.
- NYC KLGA Jun 8 78-79F YES remains represented by PT-20260607-061/PT-20260607-064/PT-20260607-068. Current readable range is roughly 30-38c versus fair 40-48%; do not add again unless a verified quote falls materially below 15c or station-specific evidence strengthens.
- London EGLC Jun 8 19C YES remains represented by PT-20260607-060 and PT-20260607-067. Current readable range is roughly 18-19c versus fair 24-34%; no duplicate unless a verified quote falls materially below 12c or hourly/station-specific evidence strengthens toward 19C.
- Atlanta KATL 80-81F is near fair after repricing; maintain PT-20260607-065 only. Atlanta 78-79F remains tracking/hedged.
- Chicago KORD Jun 8 84-85F YES remains tracking/less favorable because current official guidance and market pricing center closer to 82-83F.

## Current lessons

- Do not duplicate existing exact-bucket paper entries unless the verified price improves materially or the station-specific signal strengthens.
- A same-thesis edge can still rank highly while being a no-add because the paper book already represents it.
- Treat tabular/hourly guidance as a sanity check when a rounded daily high leaves adjacent exact buckets live.
- Track overlapping station/date clusters carefully, especially NYC, Chicago, Houston, Miami, London, and Hong Kong where exact-bucket boundaries are tight.
- A small price improvement is not enough for an add-on when visible volume is thin or localized pages conflict.
- When localized Polymarket pages disagree materially, record a price range and lower confidence unless a clean executable quote is verified.
- When an official forecast shifts by one bucket, prefer a small adjacent-bucket hedge over adding to the older thesis.
- Third same-thesis add-ons should be tiny by default even when a trigger is met.
